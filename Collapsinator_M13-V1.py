...
# NOTES:
# modified from Collapsinator.py for new oligo design (produced at beginning of sequence read 2):
...
def get_barcode(bcseq):
    ...
    spcr = "GTCGTGAT"
    # modified for oligo using M13 sequence
    # looks for M13 sequence at beginning of barcode-region sequence
    ...
    if (first == M13 and second == spcr) or inputargs['positionalbarcodes'] == True:
      counts['getbarcode_pass_exactmatch'] += 1
      return [22, 28, 36, 42]
    
    # otherwise look throughout the entire sequence for the presence of one spacer sequence
    # looks for exact match in this version as allowing substitutions in the spacer sequence will mistakenly identify the first 8 bases of the M13 sequence as a spacer sequence
    else:
      ...
      #print 'Not', bcseq
      #bcseq_ss = bcseq[26:40]

      #search for exact match in this region 
      err_spcrs = regex.findall(spcr, bcseq)
